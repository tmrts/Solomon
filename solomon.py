...
class PixelHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.user["time"] = self.sql_repr(strftime("%a, %d %b %Y %X", 
                                                   localtime()
                                                   ))
        ...
        #Experimental user tracking with bit arrays
        ...
        yield momoko.Op(self.db.execute,
                """UPDATE log_visit 
                   SET visitor_last_action_time = CURRENT_TIMESTAMP
                   WHERE visitor_id = {id}
                """.format(**self.user))
    ...
    def update_bit_array(self, identification):
        """Stores a bit array for each user and toggles the
        bit corresponding to the date user logs on.
        It is experimental.
        """
        ...
        bit_array_string = "{0:b}".format(bit_array_number | 
                                          (2 ** visitor_number)
                                          )
...
class WebSocketBaseHandler(SockJSConnection):
    ...
    def get_current_user(self, info, cookie_name):
        """Decode the encrypted cookie and retrieve the user id """
        user_id = self.to_unicode(self.decode_signed_value(
                                            cookie_secret,
                                            cookie_name,
...
class WebSocketHandler(WebSocketBaseHandler):
    """Websockets are mainly used to track 
    the online users and update the list in DB
    """
    def initialize(self):
        self.user = dict()

    #TO DO: instead of using sockjs convert it to token authentication costs less.
    @gen.coroutine
